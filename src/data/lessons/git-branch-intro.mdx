...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  <p className="leading-6">用語メモ: <Strong>ブランチ</Strong> はひとつの作業線（タイムライン）です。<Strong>main</Strong> は完成版を置く線、そこから派生した線で実験し、安全だと判断したら main へ合流させます。</p>
</div>
...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  <p className="leading-6">用語メモ: <Code>git pull --ff-only</Code> は main を最新化するコマンドです。<Strong>fast-forward（ff）</Strong> できるときだけ取り込むため、余計なマージコミットが増えません。<Code>git switch -c</Code> は「branch を作成（create）し、そのまま切り替える」近年の書きやすい書式です。</p>
</div>
...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  <p className="leading-6">用語メモ: <Strong>マージ</Strong> は別々のブランチの履歴を統合する操作です。<Strong>コンフリクト</Strong> は同じ行を別々に編集したときの競合。印を確認しながら、どの内容を採用するか選べば解決できます。</p>
</div>
...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  <p className="leading-6">用語メモ: <Code>git branch -d</Code> は「マージ済みなら削除していい」という安全装置付きのコマンドです。<Code>-D</Code> は強制削除なので、復元できない前提で使います。</p>
</div>
