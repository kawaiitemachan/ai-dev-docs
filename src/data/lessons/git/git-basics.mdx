...
<div className="mt-4 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  <p className="leading-6">事前準備: Git の考え方から触れたい場合は <TextLink href="/git-overview">Git基礎1：なぜ必要なのか？</TextLink> を、環境構築がまだの場合は <TextLink href="/git-install">Git基礎2：インストールと初期設定</TextLink> を先に完了してから読み進めるとスムーズです。</p>
</div>
...
<Text className="mt-4">
  このサイクルでは <Strong>作業ツリー</Strong>（今のファイルの中身）と<Term definition="変更予定のファイルを一時的に並べておく待機場所。">ステージ</Term>（記録待ちの変更）、<Term definition="Git が履歴を保存する金庫付きフォルダ。フォルダ直下の .git にすべての記録が入っています。">リポジトリ履歴</Term>（これまでの記録）の 3 つを行き来します。位置関係を意識しながら進めましょう。
</Text>
...
<Text className="mt-3 text-sm text-gray-600 dark:text-gray-400">
  <Term definition="Git がまだ追跡していない状態のファイル。ステージに入れると監視が始まります。">Untracked</Term> と表示されたら、<Code>git add</Code> でステージへ並べて監視を開始しましょう。
</Text>
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
...
<Text className="mt-3 text-sm text-gray-600 dark:text-gray-400">
  <Term definition="その瞬間の変更をスナップショットとして記録する操作。">コミット</Term>メッセージは短くても意味が通れば十分です。<Code>--oneline</Code> オプションはログを 1 行表示にまとめるショートビューです。
</Text>
...
<div className="mt-12 rounded-2xl border border-slate-200/80 bg-white/80 p-6 shadow-sm ring-1 ring-white/40 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40 dark:ring-white/10">
  <Subheading level={3}>図解で確認：作業ツリー → ステージ → 履歴</Subheading>
  <Text className="mt-3">
    一連の流れを視覚で押さえておくと、迷ったときに「今どこにいるのか」がすぐ分かります。作業ツリー（ワーキングツリー）からステージ、そしてリポジトリ履歴へと変更が移動する様子を下図で押さえましょう。
  </Text>
  <div className="mt-4 rounded-xl border border-slate-200/80 bg-white/70 p-4 shadow-sm ring-1 ring-white/30 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/55 dark:ring-white/10">
