...
<Heading level={1}>Git基礎：変更を記録する超基礎サイクル</Heading>
<Text>
...
<div className="mt-4 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  事前準備: Git のインストールと初期設定がまだの場合は、先にインストール編（<TextLink href="/git-install">Git基礎：インストールと初期設定</TextLink>）を完了してから読み進めるとスムーズです。
</div>
...
<div className="mt-3 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  たとえ: 作業ツリーは机の上のノート、ステージは提出前のフォルダ、リポジトリ履歴は提出ボックスと考えると、変更がどこにあるかイメージしやすくなります。
</div>
...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  補足: <Strong>mkdir</Strong> は「make directory」の略で新しいフォルダを作成します。<Strong>cd</Strong> は「change directory」で、操作するフォルダを切り替えるコマンドです。コマンドの間にある <Code>&amp;&amp;</Code> は、前の処理が成功したときだけ次を実行するシェルの連結記号です。
</div>
...
<div className="mt-3 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  たとえ: <Code>git status</Code> は作業ノートに付箋を貼って「今はここを編集中」と確認するイメージです。頻繁に見返すほど迷子になりにくくなります。
</div>
...
<div className="mt-3 rounded-md border border-emerald-200/80 bg-emerald-50 px-3 py-2 text-sm text-emerald-900 dark:border-emerald-300/50 dark:bg-emerald-400/10 dark:text-emerald-100" role="note">
  補足: GUI やエディターでファイルを追加した直後でも <Code>git status</Code> は変化を検知します。ターミナル派であれば <Code>touch</Code>（空ファイルを作るコマンド）などを使って準備しても OK です。
</div>
...
<div className="mt-3 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  たとえ: <Code>git add</Code> はレジ前のカゴに商品を集めるような感覚です。カゴに入れたものだけが次の会計（コミット）に含まれます。
</div>
...
<div className="mt-3 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  たとえ: <Code>git commit</Code> はレジで会計してレシートを受け取る瞬間です。レシート（コミットメッセージ）があると後から「何を買ったか」がすぐ分かります。
</div>
...
<div className="mt-3 rounded-md border border-sky-200/80 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-300/50 dark:bg-sky-400/20 dark:text-sky-100" role="note">
  たとえ: <Code>git restore</Code> は鉛筆の書き間違いを消しゴムで消すイメージです。コミットという「清書」をする前なら、跡を残さず戻せます。
</div>
